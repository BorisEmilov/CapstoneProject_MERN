...
router.get('/:id', auth, async (req, res) => {
    try {
        ...
        await post.remove();
        res.json({ msg: 'Post deleted succesfuly' });
    } catch (error) {
        console.error(error.message);
        if(error.kind === 'ObjectId') return res.status(404).json({ msg: 'Post not found' });
        res.status(500).send('Server Error');
...
router.put('/unlike/:id', auth, async (req, res) => {
    try {
        const post = await Post.findById(req.params.id);
        // * if the post is alredy liked
        if(post.likes.filet(like => like.user.toString() === req.user.id).length === 0){
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
    try {
        ...
        if(comment.user.toString !== req.user.id){
            return res.status(404).json({msg: 'no commento found for you'})
        }
